...
def main(golden_path, rtl_path, out_dir):
    ...
    stats_text = f"MORPHOLOGY VERIFICATION | Res: {target_w}x{target_h} | Total Pixels: {expected_len} | Mismatches: {errors} | Status: {status_str}"
    ...
    vis_path = os.path.join(out_dir, "morphology_comparison.png")
    comp_img.save(vis_path)
    
    stats_path = os.path.join(out_dir, "morphology_stats.txt")
    with open(stats_path, "w") as f:
        f.write(f"STAGE: MORPHOLOGY\n")
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python verify_morphology.py <golden.txt> <rtl.txt> <out_results_dir>")
        sys.exit(1)
